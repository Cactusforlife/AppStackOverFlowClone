...
public final class RoundImageView extends AppCompatImageView {
    ...
    public RoundImageView(final Context context) {
        this(context, null, R.styleable.RoundImageViewStyle_roundImageViewDefault);
    }

    public RoundImageView(final Context context, final AttributeSet attrs) {
        this(context, attrs, R.styleable.RoundImageViewStyle_roundImageViewDefault);
    }

    public RoundImageView(final Context context, final AttributeSet attrs, final int defStyleAttr) {
    ...
    private void init(final Context context, final AttributeSet attrs, final int defStyle) {
        ...
        final TypedArray attributes = context.obtainStyledAttributes(attrs, R.styleable.RoundImageView, defStyle, 0);

        // Check for extra features being enabled
        hasBorder = attributes.getBoolean(R.styleable.RoundImageView_riv_border, false);
        hasSelector = attributes.getBoolean(R.styleable.RoundImageView_riv_selector, false);
        shadowEnabled = attributes.getBoolean(R.styleable.RoundImageView_riv_shadow, SHADOW_ENABLED);

        // Set border properties, if enabled
        if (hasBorder) {
            final int defaultBorderSize = (int) (2 * context.getResources().getDisplayMetrics().density + 0.5f);
            setBorderWidth(attributes.getDimensionPixelOffset(R.styleable.RoundImageView_riv_borderWidth, defaultBorderSize));
            setBorderColor(attributes.getColor(R.styleable.RoundImageView_riv_borderColor, Color.WHITE));
        }

        // Set selector properties, if enabled
        if (hasSelector) {
            final int defaultSelectorSize = (int) (2 * context.getResources().getDisplayMetrics().density + 0.5f);
            setSelectorColor(attributes.getColor(R.styleable.RoundImageView_riv_selectorColor, Color.TRANSPARENT));
            setSelectorStrokeWidth(attributes.getDimensionPixelOffset(R.styleable.RoundImageView_riv_selectorStrokeWidth, defaultSelectorSize));
            setSelectorStrokeColor(attributes.getColor(R.styleable.RoundImageView_riv_selectorStrokeColor, Color.BLUE));
        }

        // Set shadow properties, if enabled
        if (shadowEnabled) {
            shadowRadius = attributes.getFloat(R.styleable.RoundImageView_riv_shadowRadius, SHADOW_RADIUS);
            shadowDx = attributes.getFloat(R.styleable.RoundImageView_riv_shadowDx, SHADOW_DX);
            shadowDy = attributes.getFloat(R.styleable.RoundImageView_riv_shadowDy, SHADOW_DY);
            shadowColor = attributes.getColor(R.styleable.RoundImageView_riv_shadowColor, SHADOW_COLOR);
     ...
     */
    public void setShadowEnabled(final boolean enabled) {
    ...
    public void updateBitmapShader() {
        ...
        if (canvasSize != image.getWidth() || canvasSize != image.getHeight()) {
            ...
            final float scale = (float) canvasSize / (float) image.getWidth();
